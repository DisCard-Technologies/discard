...
import { useCards, useCardOperations, CardWithDetails } from '../../stores/cardsConvex';
...
const CardDashboardScreen: React.FC<CardDashboardScreenProps> = ({
  ...
  return (
    <View style={styles.container}>
      {/* Command Bar - Intent-Centric AI Input */}
      {userId && (
        <CommandBar
          userId={userId}
          placeholder="Try: Fund my card with $50..."
        />
      )}
      ...
      {state.isLoading && filteredCards.length === 0 ? (
        ...
        <FlatList
          ...
          keyExtractor={(item) => item._id || item.cardId || 'no-id'}
