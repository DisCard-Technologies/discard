...
export interface DeFiProtocolRisk {
  riskId: string;
  protocolName: string;
  networkType: 'ETH' | 'POLYGON' | 'ARBITRUM';
  riskType: 'smart_contract' | 'liquidity' | 'impermanent_loss' | 'governance' | 'oracle';
  riskLevel: 'low' | 'medium' | 'high' | 'critical';
  riskDescription: string;
  mitigationMeasures: Record<string, any>;
  detectedAt: Date;
  resolvedAt?: Date;
  status: 'active' | 'monitoring' | 'resolved';
}

export interface DeFiFundingRequest {
  positionId: string;
  fundingAmount: string;
  cardContext: string;
  preferredWithdrawalMethod?: 'optimal' | 'fastest' | 'cheapest';
}
