...
- [ ] Backend Card Service Implementation (AC: 1, 2, 5, 6)
  - [ ] Create cards controller at `/apps/api/src/services/cards/cards.controller.ts`
  - [ ] Create cards service at `/apps/api/src/services/cards/cards.service.ts` 
  - [ ] Create privacy service at `/apps/api/src/services/cards/privacy.service.ts`
  - [ ] Implement POST `/api/v1/cards` endpoint for card creation
  - [ ] Implement GET `/api/v1/cards` endpoint for listing user cards
  - [ ] Implement GET `/api/v1/cards/{cardId}` endpoint for card details
  - [ ] Implement DELETE `/api/v1/cards/{cardId}` endpoint for card deletion
  - [ ] Implement PUT `/api/v1/cards/{cardId}/status` endpoint for status updates

- [ ] Database Integration with Privacy Isolation (AC: 1, 2, 5, 6)
  - [ ] Implement card creation with cryptographic isolation using card_context_hash
  - [ ] Configure row-level security policies for cards table
  - [ ] Implement encrypted card number and CVV storage with KMS keys
  - [ ] Add merchant restrictions storage and validation
  - [ ] Create deletion logging for cryptographic verification
