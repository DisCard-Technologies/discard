...
- [ ] State Management for Conversion Rates (AC: 5, 6, 7)
  - [ ] Extend crypto store at `/apps/mobile/src/stores/crypto.ts` with conversion functionality
  - [ ] Add real-time rate subscription management
  - [ ] Implement rate caching with automatic expiry
  - [ ] Add conversion quote management for slippage protection
  - [ ] Create rate history state management
...
- [ ] Testing Implementation (Testing Strategy Requirements)
  ...
  - [ ] Backend unit tests at `/apps/api/src/__tests__/unit/services/crypto/conversion.service.test.ts`
  - [ ] Integration tests at `/apps/api/src/__tests__/integration/crypto-rates.integration.test.ts`
  - [ ] Frontend component tests at `/apps/mobile/__tests__/components/crypto/ConversionCalculator.test.tsx`
  - [ ] Frontend component tests at `/apps/mobile/__tests__/components/crypto/RateComparisonView.test.tsx`
  - [ ] E2E tests at `/apps/mobile/__tests__/e2e/crypto-conversion-workflows.e2e.test.ts`
  - [ ] WebSocket connection tests for real-time rate updates
  - [ ] Mock implementations for external price APIs with error scenario coverage
  - [ ] Performance tests for rate refresh mechanisms and failover systems
...
- `/apps/api/src/services/crypto/rates.service.ts` - Rewritten with enhanced multi-exchange support, automatic failover, and WebSocket streaming
- `/apps/api/src/services/crypto/conversion.service.ts` - Created comprehensive conversion calculator with slippage protection and fee calculation
- `/apps/api/package.json` - Added decimal.js and ws dependencies for precision calculations and WebSocket support
